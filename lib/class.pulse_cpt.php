...
class Pulse_CPT {
  ...
  public static function print_form_script() {
    ...
    wp_localize_script('pulse-cpt-form', 'Pulse_CPT_Form_global', array(
  ...
  public static function the_pulse($it = null) {
    ...
    <?php if (isset($it['tags']) && is_array($it['tags'])) : ?>
      ...
      <?php endforeach; ?>
        ...
        /* expand ajax request handler */
        public static function pulse_expand() {
          global $post;
          $data = (isset($_POST['data']) ? $_POST['data'] : false);
          if ($data) {
            $qargs = Pulse_CPT::query_arguments();
            $qargs['post_parent'] = $data['pulse_id']; //add post parent to arguments
            $the_query = new WP_Query($qargs);

            while ($the_query->have_posts()) : $the_query->the_post();
              self::the_pulse(); //this needs to return json data instead
            endwhile;
          }
          die(); //remove '0' that wordpress adds otherwise
        }
